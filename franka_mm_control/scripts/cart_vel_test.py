...
class CartesianVelocityPublisher(Node):

    def __init__(self):
        super().__init__('cartesina_velocity_publisher')
        self.publisher_left = self.create_publisher(
            Float64MultiArray, 
            '/cartesian_velocity_left/commands',
            10
        )
        self.publisher_right = self.create_publisher(
            Float64MultiArray, 
            '/cartesian_velocity_right/commands',
            10
        )
...
def main(args=None):
    ...
    publisher = CartesianVelocityPublisher()
